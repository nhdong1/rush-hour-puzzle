...
class ConditionEditorDialog:
    ...
    def __init__(self, parent, condition=None, callback=None):
        ...
        Args:
            ...
            condition: Existing condition dict to edit, or None for new
            callback: Function to call with result dict when saved
        """
        self.parent = parent
    ...
    def _load_existing_template(self, template_name):
        """Load and display existing template"""
    ...
    def _open_capture_overlay(self):
        """Open fullscreen overlay for template region selection"""
        TemplateCaptureOverlay(self._on_template_captured)

    def _on_template_captured(self, image, region):
        """Callback when template region is captured"""
        try:
    ...
    def _update_template_preview(self, image):
        """Update template preview canvas"""
    ...
    def _refresh_sequence_list(self):
        """Refresh the click sequence listbox"""
    ...
    def _finalize_position_add(self, x, y, delay_ms):
        """Finalize adding a position with delay"""
    ...
    def _on_position_edited(self, idx, x, y, delay_ms):
        """Callback when position is edited"""
    ...
    def _move_position(self, direction):
        """Move selected position up (-1) or down (+1)"""
        selection = self.sequence_listbox.curselection()
        if not selection:
            return
...
class TemplateCaptureOverlay:
    """Fullscreen overlay for template region selection"""
...
class DraggablePositionPicker:
    """
    Fullscreen overlay with a draggable circle for position picking.
    User drags the circle to desired position and clicks confirm.
    """
    ...
    def _on_canvas_click(self, event):
        ...
        for item in items:
            ...
            if coords and coords[0] <= event.x <= coords[2] and coords[1] <= event.y <= coords[3]:
                return  # Let confirm handler deal with it
    ...
    def _move_circle_to(self, x, y):
        """Move circle and crosshairs to new position"""
        self.circle_x = x
        self.circle_y = y
...
class DelayInputDialog:
    """Simple dialog to input delay for a click position"""
...
class PositionEditorDialog:
    """Dialog to edit an existing click position"""
    ...
    def _open_position_picker(self):
        """Open draggable position picker"""
