...
class TestCorneringLogic(unittest.TestCase):
    """Test cornering logic - prioritize trapping red pieces"""
    ...
    def test_count_escape_routes_corner(self):
        """Red piece in corner should have fewer escape routes"""
        # Red wolf in corner (0, 0)
        red_corner = create_piece("wolf", False, 0, 0)
        self.board[0][0] = red_corner

        escapes = self.ai._count_red_escape_routes(red_corner, self.board, self.cells)
        self.assertEqual(escapes, 2, "Corner piece should have max 2 escape routes")

    def test_count_escape_routes_center(self):
        """Red piece in center should have more escape routes"""
        # Red wolf in center (1, 1)
        red_center = create_piece("wolf", False, 1, 1)
        self.board[1][1] = red_center

        escapes = self.ai._count_red_escape_routes(red_center, self.board, self.cells)
        self.assertEqual(escapes, 4, "Center piece should have 4 escape routes")

    def test_count_escape_routes_blocked(self):
        """Red piece blocked by blue pieces should have fewer routes"""
        # Red wolf at (1, 1)
        red_wolf = create_piece("wolf", False, 1, 1)
        self.board[1][1] = red_wolf
        # Block with blue pieces (blue lion can't be captured by wolf)
        self.board[0][1] = create_piece("lion", True, 0, 1)  # power 7 > 5
        self.board[1][0] = create_piece("lion", True, 1, 0)

        escapes = self.ai._count_red_escape_routes(red_wolf, self.board, self.cells)
        self.assertEqual(escapes, 2, "Blocked piece should have only 2 escapes")

    def test_hunt_score_higher_when_cornering(self):
        """Hunt score should be higher for moves that corner the enemy"""
        # Blue lion at (0, 0)
        blue_lion = create_piece("lion", True, 0, 0)
        self.board[0][0] = blue_lion
        # Red wolf at corner (0, 3) - limited escape routes
        self.board[0][3] = create_piece("wolf", False, 0, 3)

        # Move towards corner (trapping)
        score_corner = self.ai._calculate_hunt_score(
            blue_lion, 0, 1, self.board, self.cells
        )

        # Red wolf in center (1, 2) - more escape routes
        board2 = create_empty_board()
        board2[0][0] = blue_lion
        board2[1][2] = create_piece("wolf", False, 1, 2)

        score_center = self.ai._calculate_hunt_score(
            blue_lion, 0, 1, board2, self.cells
        )

        # Cornering should give higher score (or at least not lower)
        self.assertGreaterEqual(score_corner, score_center * 0.8,
            "Cornering a piece should give equal or higher hunt score")

    def test_prefers_cornering_move(self):
        """AI should prefer moves that trap red pieces in corners"""
        # Blue elephant at (1, 2)
        self.board[1][2] = create_piece("elephant", True, 1, 2)
        # Red wolf trapped in corner (0, 3) with only 2 escape routes
        self.board[0][3] = create_piece("wolf", False, 0, 3)

        # Two possible moves: (0, 2) blocks escape, (2, 2) doesn't help
        actions = self.ai.get_valid_actions(self.board, self.cells)
        ...
        move_away = None
        for a in actions:
            if a.type == ActionType.MOVE and a.piece.type == "elephant":
                if a.target_pos == (0, 2):  # Blocks wolf's escape
                    move_to_block = a
                elif a.target_pos == (2, 2):  # Moves away
                    move_away = a

        if move_to_block and move_away:
            priority_block, score_block = self.ai._get_action_priority(
                move_to_block, self.board, self.cells
            )
            priority_away, score_away = self.ai._get_action_priority(
                move_away, self.board, self.cells
            )

            # Blocking escape should have better (lower) priority or higher score
            self.assertTrue(
                priority_block < priority_away or
                (priority_block == priority_away and score_block >= score_away),
                "Move that blocks escape should be prioritized"
            )
...
def run_all_tests():
    ...
    suite.addTests(loader.loadTestsFromTestCase(TestCorneringLogic))  # New cornering tests
